...
PointToPointNetDevice::GetQueuePackets ()
{
  NS_LOG_FUNCTION(this);
...
PointToPointNetDevice::TransmitStart (Ptr<Packet> p)
{
  ...
  NS_LOG_LOGIC ("Transmitting packet of size " << p->GetSize ());
  NS_LOG_LOGIC ("Schedule TransmitCompleteEvent in " << txCompleteTime.GetSeconds () << "sec  - Data Rate = " << m_bps);
  ...
  uint16_t protocolNumber)
{
  ...
  m_macTxTrace (packet);
//  cout << "Sending a packet at netdevice. queue already contains " << m_queue->GetQueue().size() << endl;
  //
...
uint64_t
PointToPointNetDevice::GetTotalBytesTransmitted()
{
